...
def build_transition_velocity_tavv():
    ...
    after_friction = np.expand_dims(after_friction, (0, 1, 2))  # shape = (1, 1, 1, n_velocity) as we broadcast over t, a, p

    push__ext = np.expand_dims(push_effect, (0, -1))  # shape = (1, n_timestep, n_velocity, 1)
    action_effect = np.vstack((np.zeros_like(push__ext), push__ext))  # shape = (n_action, n_timestep, n_velocity, 1)

    own_force__ext = np.expand_dims(own_force, (0,  -1))  # shape = (1, n_timestep, n_velocity, 1)

    new_v = after_friction + action_effect + own_force__ext  # shape = (n_action, n_timestep, n_velocity, n_velocity)
...
def build_transition_position_pvp():
    ...
    for p_idx, p in enumerate(position):
        for v_idx, v in enumerate(velocity):
            tr[p_idx, v_idx, :] = norm.pdf(position, loc=p + (1 / n_timestep) * v, scale=sigma_transition_position)
    return normalize_last_column(tr)
